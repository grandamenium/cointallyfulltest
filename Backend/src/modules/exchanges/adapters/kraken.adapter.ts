...
@Injectable()
export class KrakenAdapter extends BaseExchangeAdapter implements IExchangeAdapter {
  getExchangeName(): string {
    return 'kraken';
  }

  private createExchange(credentials: ExchangeCredentials): ccxt.kraken {
  ...
  async testConnection(credentials: ExchangeCredentials): Promise<boolean> {
    try {
      ...
      this.handleApiError(error, 'Kraken connection test');
    }
  }
  ...
  async syncAccounts(credentials: ExchangeCredentials): Promise<ExchangeAccount[]> {
    try {
      ...
      this.handleApiError(error, 'Kraken sync accounts');
    }
  }
    ...
    try {
      ...
      for (const currency of currencies) {
        try {
          ...
          this.logger.warn(`Kraken: Failed to fetch transactions for ${currency}: ${error.message}`);
        }
      }
      ...
      this.handleApiError(error, 'Kraken sync transactions');
    }
  }
}
