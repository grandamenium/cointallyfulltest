...
export class BybitAdapter extends BaseExchangeAdapter implements IExchangeAdapter {
  ...
  private createExchange(credentials: ExchangeCredentials): ccxt.bybit {
  ...
  async testConnection(credentials: ExchangeCredentials): Promise<boolean> {
    try {
      ...
      this.handleApiError(error, 'Bybit connection test');
    }
  }
  ...
  async syncAccounts(credentials: ExchangeCredentials): Promise<ExchangeAccount[]> {
    try {
      ...
      this.handleApiError(error, 'Bybit sync accounts');
    }
  }
    ...
    try {
      ...
      for (const currency of currencies) {
        try {
          ...
          this.logger.warn(`Bybit: Failed to fetch transactions for ${currency}: ${error.message}`);
        }
      }
      ...
      this.handleApiError(error, 'Bybit sync transactions');
    }
  }
}
