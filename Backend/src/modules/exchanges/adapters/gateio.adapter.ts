...
@Injectable()
export class GateioAdapter extends BaseExchangeAdapter implements IExchangeAdapter {
  getExchangeName(): string {
    return 'gateio';
  }

  private createExchange(credentials: ExchangeCredentials): ccxt.gateio {
  ...
  async testConnection(credentials: ExchangeCredentials): Promise<boolean> {
    try {
      ...
      this.handleApiError(error, 'Gate.io connection test');
    }
  }
  ...
  async syncAccounts(credentials: ExchangeCredentials): Promise<ExchangeAccount[]> {
    try {
      ...
      this.handleApiError(error, 'Gate.io sync accounts');
    }
  }
    ...
    try {
      ...
      for (const currency of currencies) {
        try {
          ...
          this.logger.warn(`Gate.io: Failed to fetch transactions for ${currency}: ${error.message}`);
        }
      }
      ...
      this.handleApiError(error, 'Gate.io sync transactions');
    }
  }
}
