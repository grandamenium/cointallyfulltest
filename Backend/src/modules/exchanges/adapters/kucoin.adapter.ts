...
@Injectable()
export class KucoinAdapter extends BaseExchangeAdapter implements IExchangeAdapter {
  getExchangeName(): string {
    return 'kucoin';
  }

  private createExchange(credentials: ExchangeCredentials): ccxt.kucoin {
  ...
  async testConnection(credentials: ExchangeCredentials): Promise<boolean> {
    try {
      ...
      this.handleApiError(error, 'KuCoin connection test');
    }
  }
  ...
  async syncAccounts(credentials: ExchangeCredentials): Promise<ExchangeAccount[]> {
    try {
      ...
      this.handleApiError(error, 'KuCoin sync accounts');
    }
  }
    ...
    try {
      ...
      for (const currency of currencies) {
        try {
          ...
          this.logger.warn(`KuCoin: Failed to fetch transactions for ${currency}: ${error.message}`);
        }
      }
      ...
      this.handleApiError(error, 'KuCoin sync transactions');
    }
  }
}
