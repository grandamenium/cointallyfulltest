...
@Injectable()
export class BinanceAdapter extends BaseExchangeAdapter implements IExchangeAdapter {
  getExchangeName(): string {
    return 'binance';
  }

  private createExchange(credentials: ExchangeCredentials): ccxt.binance {
  ...
  async testConnection(credentials: ExchangeCredentials): Promise<boolean> {
    try {
      ...
      this.handleApiError(error, 'Binance connection test');
    }
  }
  ...
  async syncAccounts(credentials: ExchangeCredentials): Promise<ExchangeAccount[]> {
    try {
      ...
      this.handleApiError(error, 'Binance sync accounts');
    }
  }
    ...
    try {
      ...
      for (const currency of currencies) {
        try {
          ...
          this.logger.warn(`Binance: Failed to fetch transactions for ${currency}: ${error.message}`);
        }
      }
      ...
      this.handleApiError(error, 'Binance sync transactions');
    }
  }
}
